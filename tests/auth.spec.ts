...
test.describe("Authentication - Login", () => {
  ...
  test("Should log in successfully with valid credentials", async ({
    ...
    await expect(page.locator("#chat-link")).toBeVisible();
  });

  // Test 6: Incorrect password
  test("Should show error for incorrect password", async ({ page }) => {
    await page.goto(`${BASE_URL}/login`);
    await page.fill("#login-email", "minhbqgcd210325@gmail.com");
    await page.fill("#login-password", "incorrect-password");
    await page.click("#login-button");
...
test.describe("Authentication - Sign Up", () => {
  ...
  // // Test 6: Unsuccessful signup - email already exists
  // test("Should show error when signup with existing email", async ({
  //   page,
  // }) => {
  //   await page.goto(`${BASE_URL}${ROUTE_PATH.SIGNUP}`);
  //   await page.fill("#signup-lastname", "Nguyen");
  //   await page.fill("#signup-firstname", "Minh");
  //   await page.fill("#signup-email", "minhbqgcd210325@fpt.edu.vn");
  //   await page.fill("#signup-password", "string");
  //   await page.click("#signup-button");

  //   // await page.waitForTimeout(5000);
  //   await expect(page.locator("body")).toContainText("Tài khoản đã tồn tại");
  // });
});
