...
test.describe("Users Page", () => {
  ...
  // test("should show loading state when fetching users", async () => {
  //   await expect(page.locator(".loader")).toBeVisible();
  // });

  // test("should display an error message if data fetching fails", async () => {
  //   // Intercept the request and simulate a failure
  //   await page.route("**/users**", async (route) => await route.abort());

  //   await page.reload();
  //   await expect(page.getByText("Lỗi, vui lòng thử lại")).toBeVisible();
  // });

  test("should allow selecting a category", async () => {
    await page.click("text=Thể thao");
